...
def show_user_incomes(all_user_data):
    # result = [all_user_data[date]['income'] for date in all_user_data] ??
    result = []
    for date in all_user_data:
...
def show_user_data_per_date(date, all_user_data):
    ...
    result = [(amount, label, INC_STR if category == 'income' else EXP_STR)
              ...
              for amount, label in all_user_data[date][category]]
    return result
    # data_on_date = all_user_data[date]
    # result = []
    # for category in data_on_date:
    #     for amount, label in data_on_date[category]:
    #         category_str = 'New Income' if category == 'income' else 'New Expense'
    #         result.append((amount, label, category_str))
