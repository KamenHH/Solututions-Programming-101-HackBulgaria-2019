...
class TestDeepOps(unittest.TestCase):
    def setUp(self):
        self.simple_dict = DataGenerator({'a': 1, 'b': 2})
        
        self.nested_dicts = DataGenerator({
            'a': {
                'a': 1,
                'b': {
                    'a': 3
        ...
        self.simple_list_with_dict = DataGenerator([{'a': 1, 'b': 2}])

        self.nested_dicts_and_lists = DataGenerator([
            {
                'a': 1,
                'b': [
                    {
    ...
    def test_deep_update_simple_dict_dfs(self):
        ...
        self.assertEqual(self.simple_dict.data, expected_result)
    ...
    def test_deep_update_nested_dicts_dfs(self):
        ...
        self.assertEqual(self.nested_dicts.data, expected_result)
    
    def test_deep_update_simple_list_with_dict_dfs(self):
        do_obj = DeepOps(self.simple_list_with_dict)
        do_obj.deep_update('b', 3)
        expected_result = [{'a': 1, 'b': 3}]
        self.assertEqual(self.simple_list_with_dict.data, expected_result)
    ...
    def test_deep_update_nested_dicts_and_lists_dfs(self):
        ...
        self.assertEqual(self.nested_dicts_and_lists.data, expected_result)
