...
class GraphBuilder:
    ...
    in a specific according to the algorithm (DFS, BFS) used."""
    ...
    def bfs(self):
        ...
        def bfs_helper(q, struct):
            ...
            for obj in data:
                ...
                if GraphBuilder.is_dict(data[obj]): # and data[obj] not in q:
                    q.append(data[obj])
                elif GraphBuilder.is_iterable(data[obj]): # and data[obj] not in q::
        ...
        def iterables_helper(iterable, queue):
            """Enques the object if it's a dict or
            calls self recursively if @obj is an iterable (list, set, tuple..)."""
            for obj in iterable:
                if GraphBuilder.is_dict(obj):
                   queue.append(obj)
